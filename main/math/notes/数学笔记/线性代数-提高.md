...
3. 同解问题
   * 齐次：$Ax = 0,Bx = 0$
     * $\Leftrightarrow$ 三秩相同：$r(A) = r(B) = r(A|B)$
     * $\Leftrightarrow$ 行向量组等价
     * $\Leftrightarrow$ 存在矩阵 $P,Q$，使得 $B = PA,A = QB$
   * 非齐次：$Ax = \beta,Bx = \gamma$
     * $\Leftrightarrow$ 四秩相同：$r(\begin{bmatrix} A & \beta \\ B & \gamma \end{bmatrix}) = r(\begin{bmatrix} A \\ B \end{bmatrix}) = r(A) = r(B)$
     * $\Leftrightarrow$ 各自导出组同解，且非齐次有公共解
...
* **A 可相似对角化 $\Leftrightarrow$ 存在 n 个线性无关的特征向量**
* 对矩阵的每个特征值，该特征值是几重根，其代数重数就是几；计算 $n - r(\lambda E - A)$ 得到几何重数。若存在一个特征值几何重数不等于代数重数，则矩阵不能相似对角化，否则矩阵可以相似对角化
