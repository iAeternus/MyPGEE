...
5. 无理式的运算
   ...
   * 只要根号下为偶数次方，化简出来就必须要加绝对值，如 $\sqrt[8]{x^{12}} = \sqrt[8]{|x|^{12}} = |x|^{\frac{12}{8}} = |x|^{\frac{3}{2}} = \sqrt{|x|^3} = \begin{cases} x\sqrt{x} & x \gt 0 \\ -x\sqrt{-x} & x \le 0 \end{cases}$
...
4. **若 $f(x)$ 在 $x_0$ 可导且 $f(x_0)\ne 0$，则 $|f(x)|$ 在 $x_0$ 可导**
5. **若 $f(x)$ 在 $x_0$ 可导且 $f(x_0) = 0$ 且 $f'(x_0) \ne 0$（可测到变化率），则 $|f(x)|$ 在 $x_0$ 不可导**
...
3. $\Delta y = A\Delta x + o(\Delta x) = f'(x_0)\Delta x + o(\Delta x)$
4. $dy = A\Delta x = y'dx$ 为线性主部
5. 本质是用 $x_0$ 处切线的线性增量近似函数真实的增量 $\Delta y$，二者相差一个高阶无穷小
...
则存在 $\xi \in (a, b)$，使得 $f(b) - f(a) = f'(\xi)(b - a)$，或者写成 $f'(\xi) = \cfrac{f(b) - f(a)}{b - a}$
...
则存在 $\xi \in (a, b)$，使得 $\cfrac{f(b) - f(a)}{g(b) - g(a)} = \cfrac{f'(\xi)}{g'(\xi)}$
...
2. 估值定理：设 $M,m$ 分别是 $f(x)$ 在 $[a, b]$ 上的最大值和最小值，$L$ 为区间 $[a, b]$ 的长度，则有
   $$
   mL \le \int_a^b f(x) dx \le ML
   $$

3. 积分中值定理：设 $f(x)$ 在区间 $[a, b]$ 上连续，则至少存在一点 $\xi \in [a, b]$，使得
   $$
   \int_a^b f(x) dx = f(\xi)(b - a)
   $$

4. 积分中值定理的推广：若 $f(x)$ 和 $g(x)$ 在 $[a, b]$ 上连续，且 $g(x)$ 在 $[a, b]$ 上不变号，则至少存在一点 $\xi \in (a, b)$，使得
   $$
   ...
   >G(x) = \int_a^x g(t) dt
   >$$
   >这两个函数均在 $[a, b]$ 上可导，$g(x)$ 不变号，从而不为零，也就不能同时为零，并且 $G(x) \ne 0 = G(a)$
   >
   >根据柯西中值定理，$\exists \xi \in (a, b)$，使得
   >$$
   >\cfrac{F(b) - F(a)}{G(b) - G(a)} = \cfrac{F(b)}{G(b)} = \cfrac{F'(\xi)}{G'(\xi)} = \cfrac{f(\xi)g(\xi)}{g(\xi)} = f(\xi)
   >$$
   >于是 $F(b) = f(\xi)G(b)$，即
   >$$
   >\int_a^b f(x)g(x) dx = f(\xi) \int_a^b g(x) dx
...
1. “平面上的曲边梯形”的形心坐标公式
   ...
   ![](./assets/平面上的曲边梯形的形心坐标公式.jpg)

2. 平均值：$x \in [a, b]$，函数 $f(x)$ 在 $[a, b]$ 上的平均值为 $\overline{f} = \cfrac{1}{b - a} \int_a^b f(x) dx$
...
$dF = G\cfrac{\mu m}{(a - x)^2} dx$，于是 $F = \int_{-l}^0 G\cfrac{\mu m}{(a - x)^2} dx$
...
2. 在直角坐标系下，若把 $x$ 与 $y$ 对调后，区域 $D$ 不变（或区域 $D$ 关于 $y = x$ 对称），则
   $$
   \iint_D f(x, y) d\sigma = \iint_D f(y, x) d\sigma = I \\
   I = \cfrac{1}{2} \Big [\iint_D f(x, y) d\sigma + \iint_D f(y, x) d\sigma \Big]
   $$
   
3. 一个积分难计算，两个积分加起来式子简单，就用轮换对称性
...
形如
$$
\cfrac{dy}{dx} = \psi(\cfrac{y}{x})
$$
令 $u = \cfrac{y}{x}$，则 $y = us$，$\cfrac{dy}{dx} = u + x\cfrac{du}{dx}$，于是原方程可化为
$$
x\cfrac{du}{dx} + u = \psi(u)
$$
即转化为可分离变量型微分方程
...
1. $y'' = f(x, y')$ 型（不显含未知函数 $y$）
   * 令 $y' = p$，$y'' = p'$，则原方程变为一阶方程 $\cfrac{dp}{dx} = f(x, p)$（赶尽杀绝 $y$）
2. $y'' = f(y, y')$ 型（不显含自变量 $x$）
   * 令 $y' = p$，$y'' = \cfrac{dp}{dx} = \cfrac{dp}{dy}\cfrac{dy}{dx} = \cfrac{dp}{dy}\cdot p$，原方程变为一阶方程 $p\cfrac{dp}{dy} = f(y, p)$（斩草除根 $x$）
3. $y'' = f(y')$ 型（既不显含 $y$，又不显含 $x$）：按 $1$ 的办法，即不显含 $y$ 来处理
...
5. 组合数：$n\cdot C^n_m=m\cdot C^{n-1}_{m-1}$，$C_n^k = \cfrac{A_n^k}{A_k^k}$
6. $x^x = e^{x\ln{x}}$
