...
采用归并的思想对磁盘元素进行排序，IO 次数为算法效率的衡量指标，对于长度为 $m$ 和 $n$ 的两个归并段，从读入输入缓冲区和从输出缓冲区写入磁盘所需总共的 IO 次数为 $2(m + n)$（可以画一棵归并树，节点值为归并段长度，总 IO 子树等于所有非叶子节点权重之和）
...
要使 $n_k$ 为整数，考虑补充虚节点，则 $(m + x - 1) \% (k - 1) == 0$，即可求出 $x$
